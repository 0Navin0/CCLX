...
# keeping everything in here independent of cosmology
class CacciatoHOD(ccl.halos.HaloProfileHOD):
    ...
    Note: 
        ...
        HODs are unitless. But the characteristic masses and luminosities hold the
        information of units and scalings involved. Here, we require the masses and
        Luminosities in Msun/h and Lsun/h^2 respectively to make sure that the
        Cacciato+13 CLF parameters can be directly passed.

        CCL HMF is given as dn/dlog10(m[Msun]), so make sure to work with
        masses in Msun units out of the HOD definition and use logarithmic base 10
        mass bins for integration.

        To use the Cacciato+2013 HOD parameters as is, all the methods expect the
        mass to be in Msun/h. So an internal conversion of pyCCL's Msun mass is
        implemented where necessary.

        For Cacciato specific usecase, we do not vary free parameters in redshift dependent manner.

        eta: multiplicative factor for parent halo concentration
            set eta=1. to use a fixed c-M relation for parent halo.
        R_s: multiplicative factor for satellite/subhalo occupation
            set R_s=1. to populate satellites following the same density distr as the parent's NFW.
    """
    def __init__(
        self,
        *,
    ...
    def update_parameters(self, **kwargs):
        ...
        # concentration bias parameters (not used in Cacciato+13, but can be used in the future for satellite profile modifications)
        self.bg_0 = kwargs.get('bg_0', getattr(self, 'bg_0', 1.))
        self.bg_p = kwargs.get('bg_p', getattr(self, 'bg_p', 0.))
        self.bmax_0 = kwargs.get('bmax_0', getattr(self, 'bmax_0', 1.))
        self.bmax_p = kwargs.get('bmax_p', getattr(self, 'bmax_p', 0.))
        ...
        # parent halo concentration scaling
        self.eta = kwargs.get('eta', getattr(self, 'eta', 0.))
